...
describe('ImapClientAdapter', () => {
  ...
  test('正常系: エラーイベント発生時に再接続が試行されること', async () => {
    ...
    // イベントが発火されたことを検出するためのPromiseを作成
    const connectionLostPromise = new Promise<void>(resolve => {
      adapter.once('connectionLost', () => {
        resolve();
      });
    });
  ...
  test('正常系: KeepAliveエラー時に再接続が試行されること', async () => {
    ...
    // keepAliveのエラーハンドリングを手動で呼び出す
    const keepAliveTimer = (adapter as any).keepAliveTimer;
    clearInterval(keepAliveTimer);

    try {
      await mockImapFlowInstance.noop();
    } catch (error) {
      // 実際のKeepAlive処理を手動で呼び出す
