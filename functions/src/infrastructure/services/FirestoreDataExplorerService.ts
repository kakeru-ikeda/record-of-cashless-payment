...
export class FirestoreDataExplorerService {
     ...
     * 指定された年月のデータを探索
     */
    private async exploreMonthData(year: string, month: string, startDate: Date, endDate: Date): Promise<CardUsageDocument[]> {
        const cardUsageDocuments: CardUsageDocument[] = [];
        ...
        try {
            ...
            for (const term of terms) {
                ...
                try {
                    const termData = await this.exploreTermData(termPath, year, month, term, startDate, endDate);
     ...
     * 指定されたterm（週）のデータを探索
     */
    private async exploreTermData(termPath: string, year: string, month: string, term: string, startDate: Date, endDate: Date): Promise<CardUsageDocument[]> {
        const cardUsageDocuments: CardUsageDocument[] = [];
     ...
     * 指定された日のデータを探索
     */
    private async exploreDayData(dayPath: string, year: string, month: string, term: string, day: string): Promise<CardUsageDocument[]> {
        const cardUsageDocuments: CardUsageDocument[] = [];
                    ...
                    if (documentData && typeof documentData.amount === 'number') {
                        cardUsageDocuments.push({
                            ...
                            data: {
                                ...
                                datetime_of_use: documentData.datetime_of_use?.toDate ? documentData.datetime_of_use.toDate() : new Date(documentData.datetime_of_use)
                            },
                            params: {
                                year,
                                month,
    ...
    private async getAvailableDays(termPath: string): Promise<string[]> {
        try {
            ...
            return dayCollections.map(collection => collection.id).sort((a, b) => parseInt(a) - parseInt(b));
        } catch (error) {
            return [];
        }
    }
