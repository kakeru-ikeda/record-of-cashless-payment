...
レポートのアラートしきい値はFirestoreで管理されています。Firebase Consoleから以下のドキュメントを作成してください：

**パス**: `config/report_thresholds`
...
- **weekly**: 週次レポートのしきい値（円）
- **monthly**: 月次レポートのしきい値（円）
- **level1, level2, level3**: アラートレベル（level1 < level2 < level3の順序で設定）

これらの値はFirestore上で変更することで、アプリケーションの再デプロイなしにしきい値を調整できます。
