...
import { WeeklyReportNotification } from '../types/WeeklyReportNotification';
...
export class DiscordWebhookNotifier implements DiscordNotifier {
    ...
    /**
     * Discord Webhookを使用して週次レポートを通知する
     * @param data 週次レポート情報
     * @returns 通知の成功または失敗を表すブール値
     */
    async notifyWeeklyReport(data: WeeklyReportNotification): Promise<boolean> {
        try {
            ...
            console.log('📤 週次レポートの通知を送信しています...');

            const response = await axios.post(this.webhookUrl, { embeds });

            if (response.status === 204 || response.status === 200) {
                console.log('✅ 週次レポートの通知を送信しました');
                return true;
            } else {
                console.error('❌ 週次レポートの通知の送信に失敗しました。ステータスコード:', response.status);
                return false;
            }
        } catch (error) {
            console.error('❌ 週次レポートの通知の送信中にエラーが発生しました:', error);
            return false;
        }
    }
}